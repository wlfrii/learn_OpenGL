...
std::string proj_name = "1_06_learn_coordinate";
...
int main(int argc, char* argv[])
{
    std::cout << proj_name << ":\n"
        ...
        "\t - 2, render a square consist of 36 vertices and texcoords and rotated with "
        "time goes on. Each plane of the square is rendered with two textures. NOTE, "
        "this dynamic square may looks weird\n"
        "\t - 3, based on # 2, enable the depth test so that OpenGL can know which "
        "pixel should be show and which should be hidden\n"
        "\t - 4, based on # 3, change the position of square, then render 10 fixed "
        "squares simultaneously\n"
        "\t - 5, based on # 3, change the position of square, then render 10 rotated "
        "squares simultaneously\n"
        "Press 'Esc' to exit.\n";
    if (argc < 2) {        
        return 0;
